	...
	addListener(opts, listener) {
		...
		const add = dir => {
			...
			if (!this.children[dir]) {
				this.children[dir] = new Watcher(path.join(this.path, dir));
			}
		...
		const wrapper = evt => {
			const subdir = path.basename(evt.file);
 ...
 */
export function watch(path_, opts, listener) {
	...
	const segments = path_.split(path.sep);
	while (!path.basename(segments[0])) {
		segments.shift();
	}

	// make sure the root watcher exists
	if (!rootWatcher) {
		rootWatcher = new Watcher(path.resolve('/'));
	...
	for (const filename of segments) {
		...
		if (!watcher.children[filename]) {
			watcher.children[filename] = new Watcher(path.join(watcher.path, filename));
		}
